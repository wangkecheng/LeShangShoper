...
- (instancetype)initWithCustomView:(UIView *)customView{
    ...
    return [self initWithCustomView:customView isTouchEdgeHide:NO showAnimate:YES animatingStyle:DSAlertAnimatingStyleScale];
}
- (instancetype)initWithCustomView:(UIView *)customView animatingStyle:(DSAlertAnimatingStyle)animateStyle{
    
    return [self initWithCustomView:customView isTouchEdgeHide:NO showAnimate:YES animatingStyle:animateStyle];
}
#pragma mark - ***** 初始化自定义View
- (instancetype)initWithCustomView:(UIView *)customView isTouchEdgeHide:(BOOL)isTouchEdgeHide showAnimate:(BOOL)showAnimate animatingStyle:(DSAlertAnimatingStyle)animateStyle
{
    if (self = [super initWithFrame:CGRectZero]){
        _isTouchEdgeHide = isTouchEdgeHide;
        _showAnimate = showAnimate;
        _animatingStyle = animateStyle;
        _subView = customView;
        _subView.translatesAutoresizingMaskIntoConstraints = false;
        [self setupUI];
        [self ds_showAlertView];
    }
    return self;
}
...
- (void)setupUI
{
    ...
    if ( !self.subView.translatesAutoresizingMaskIntoConstraints ) {
        ...
        CGFloat marginTop = _subView.center.y - CGRectGetHeight(_subView.frame)/2.0;
        [self.subView autoPinEdgeToSuperviewEdge:ALEdgeTop withInset:marginTop relation:NSLayoutRelationEqual];
    }
